...
1. **Orientacja bębna do druku**:
   ...
   - Taka orientacja gwarantuje:
     ...
     - Kołnierz tylny ze stożkową fazą drukuje się na samej górze czysto i bez żadnych podpór.

2. **Druk bez podpór (No Supports Required)**:
   - Dzięki usunięciu sztucznej szyjki przed krzywkami (czerwona strzałka) oraz przejściu z dysku czołowego przez podcięcie do kołnierza głównego, model można drukować **bez podpór** lub z minimalnym supportem pod krawędzią kołnierza głównego (kąt nawisu wynosi 90° na odcinku 2.7 mm, co wiele nowoczesnych drukarek mostkuje bez problemu; dla idealnej krawędzi zalecany jest lekki support typu tree / organic pod kołnierzem głównym).

3. **Grawerunek na pierwszej warstwie**:
...
4. **Trzpień testowy (`mating_shaft_reference.stl`)**:
